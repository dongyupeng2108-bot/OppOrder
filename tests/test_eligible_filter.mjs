...
// ── Test 1: event_time filter ─────────────────────────────────────────────────
await test('1. event_time: PASS (valid ISO8601) / FAIL (null) / FAIL (empty string)', () => {
...
// ── Test 2: TTL30d filter ─────────────────────────────────────────────────────
await test('2. TTL30d: PASS (within 30 days) / FAIL (exceeds 30 days)', () => {
  // PASS: 10 days out
...
// ── Test 3: yes_price range filter ────────────────────────────────────────────
await test('3. yes_price: PASS (0.5) / FAIL (0.02 < 0.05) / FAIL (0.98 > 0.95)', () => {
  // PASS
...
await test('4. Candidate cap: 60 valid markets → output ≤ 50', () => {
  // All markets pass all 3 hard filters; cap should limit to 50
  const markets = Array.from({ length: 60 }, (_, i) => validMarket({
    title: `Will event ${i + 1} happen?`,
...
// ── Test 5: scan_stats structure verification ─────────────────────────────────
await test('5. scan_stats: filter_breakdown keys + numeric counters correct', () => {
  const markets = [
  ...
  assert('settled_market'     in fb, 'filter_breakdown.settled_market missing');
  ...
  assert(!('no_numeric_threshold' in fb), 'no_numeric_threshold should be removed');
  ...
  assert.equal(scan_stats.fetched_total,   3,                 'fetched_total should be 3');
