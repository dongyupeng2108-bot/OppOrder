...
if ($Mode -eq "Integrate") {
    if ($AutoPR) {
        ...
        if ($DirtyStatus) {
            ...
            # Use explicit double quotes to ensure Start-Process passes it as a single argument to powershell.exe
            ...
            Write-Host "DEBUG: CommitCmd = $($CommitCmd -join ' ')" -ForegroundColor Yellow
