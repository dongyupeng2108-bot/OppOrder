...
const DEFAULT_TASK_ID = '260406_009';
const DEFAULT_BASE_URL = 'http://localhost:54123';

const parseArgs = () => parseVerifyArgs({
  defaultTaskId: DEFAULT_TASK_ID,
  defaultBaseUrl: DEFAULT_BASE_URL,
  defaultOutputSuffix: 'truth_audit_json_guard_remaining_endpoints_260406_009',
  defaultSampleName: 'json_guard_remaining_endpoints'
});
...
const main = async () => {
  ...
  try {
    ...
    const targets = ['/trading/manual', '/strategies/start', '/strategy-runner/deploy'];
    const malformedResults = [];
    const arrayResults = [];
    for (const p of targets) {
      malformedResults.push(await requestPath(baseUrl, p, 'POST', '{ bad_json', headers));
      arrayResults.push(await requestPath(baseUrl, p, 'POST', '[]', headers));
    }

    const malformedBodies = malformedResults.map((r) => parseJson(r.body));
    const arrayBodies = arrayResults.map((r) => parseJson(r.body));

    checks = {
      malformed_json_returns_400_all_targets: malformedResults.every((r) => r.status === 400),
      malformed_json_error_message_stable: malformedBodies.every((b) => b.error === 'invalid json payload'),
      non_object_json_returns_400_all_targets: arrayResults.every((r) => r.status === 400),
      non_object_json_error_message_stable: arrayBodies.every((b) => b.error === 'invalid json payload type'),
  ...
  const firstBreakLayer = pass ? 'NONE_CHAIN_PASS' : 'json_guard_remaining_endpoints';
  const standard = buildStandardResult({
    scriptName: 'truth_audit_json_guard_remaining_endpoints_260406_009',
    taskId: args.taskId,
    ...
    rawExcerpt: {
      fail_to_pass: {
        before: 'remaining_endpoints_json_guard_incomplete',
        after: pass ? 'remaining_endpoints_json_guard_complete' : 'remaining_endpoints_json_guard_still_broken'
      },
