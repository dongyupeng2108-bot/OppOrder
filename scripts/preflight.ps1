...
# Filter out current task's evidence files (which might be generated by previous steps like Workspace Healer)
$IgnoredPatterns = @("workspace_healer_$TaskId.json", "preflight_attestation_$TaskId.json", "notify_$TaskId.txt", "result_$TaskId.json", "run_$TaskId.log", ".budget_$TaskId.json", "ci_parity_$TaskId.json", "dod_evidence_$TaskId.txt", "git_meta_$TaskId.json", "generate_evidence_$TaskId.mjs")
$FilteredStatus = $GitStatus | Where-Object { 
    $Line = $_
    $IsIgnored = $false
    foreach ($Pattern in $IgnoredPatterns) {
        if ($Line -match $Pattern) { $IsIgnored = $true; break }
    }
    -not $IsIgnored
}
