...
const regeneratedByTaskArg = getArgValue('regenerated_by_task');
const generatedFromHeadArg = getArgValue('generated_from_head');
...
const runtimeHeadCommit = generatedFromHeadArg
    || process.env.PR_HEAD_SHA
    || process.env.GITHUB_SHA
    || process.env.EXPECTED_PR_HEAD_SHA
    || liveGitMeta?.commit
    || gitMeta.commit;
const regeneratedByTask = regeneratedByTaskArg || process.env.REGENERATED_BY_TASK || taskId;
...
Task ID: ${taskId}
Header: ${taskHeader}
Artifact Task ID: ${taskId}
Regenerated By Task: ${regeneratedByTask}
Generated From Head: ${String(runtimeHeadCommit || '').trim()}
