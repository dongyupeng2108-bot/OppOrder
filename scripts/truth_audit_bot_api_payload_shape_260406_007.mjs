...
const DEFAULT_TASK_ID = '260406_007';
const DEFAULT_BASE_URL = 'http://localhost:54123';

const parseArgs = () => parseVerifyArgs({
  defaultTaskId: DEFAULT_TASK_ID,
  defaultBaseUrl: DEFAULT_BASE_URL,
  defaultOutputSuffix: 'truth_audit_bot_api_payload_shape_260406_007',
  defaultSampleName: 'bot_api_payload_shape'
});
...
const main = async () => {
  ...
  try {
    ...
    const resReload = await requestPath(baseUrl, '/config/reload', 'POST', arrayPayload, headers);
    const resBotConfig = await requestPath(baseUrl, '/bot/config', 'POST', arrayPayload, headers);
    const resBotStart = await requestPath(baseUrl, '/bot/start', 'POST', arrayPayload, headers);
    ...
    checks = {
      array_payload_reload_returns_400: resReload.status === 400,
      array_payload_bot_config_returns_400: resBotConfig.status === 400,
      array_payload_bot_start_returns_400: resBotStart.status === 400,
      payload_shape_error_message_stable: bodyReload.error === 'invalid json payload type'
        && bodyBotConfig.error === 'invalid json payload type'
        && bodyBotStart.error === 'invalid json payload type',
  ...
  const firstBreakLayer = pass ? 'NONE_CHAIN_PASS' : 'bot_api_payload_shape';
  const standard = buildStandardResult({
    scriptName: 'truth_audit_bot_api_payload_shape_260406_007',
    taskId: args.taskId,
    ...
    rawExcerpt: {
      fail_to_pass: {
        before: 'non_object_json_payload_accepted',
        after: pass ? 'non_object_json_payload_rejected' : 'payload_shape_validation_still_broken'
      },
