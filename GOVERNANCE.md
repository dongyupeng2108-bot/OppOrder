...
```
【依赖接口确认 — 动手前必须执行，结果粘贴后再进入 STEP 1】

对每个跨模块依赖，执行以下命令并粘贴输出：

1. 查被调用模块的 export 接口：
   Get-Content <被调用模块路径> | Select-String "export"

2. 查调用方当前使用的接口名：
   Get-Content <调用方路径> | Select-String "<接口关键字>"

3. 查相关配置字段是否存在：
   Get-Content <配置文件路径> | Select-String "<字段名>"

确认后在此填写：
- 被调用模块实际 export 接口：___
- 调用方当前使用的接口名：___
- 接口是否一致：是/否（不一致时胶囊必须同时修复两侧）
- 配置字段是否存在：是/否（不存在时胶囊必须加防御解构）
```
...
| 场景 | 是否需要接口确认步骤 |
|------|---------------------|
| 修改模块 A，同时有其他模块调用 A | ✅ 必须 |
| 新建模块 B，被已有模块 C 调用 | ✅ 必须 |
| 修改配置 JSON，被代码解构 | ✅ 必须 |
| 纯新增文件，无跨模块调用 | ❌ 可跳过 |
| 仅修改治理文档 | ❌ 可跳过 |
...
- PM 出胶囊时必须识别是否存在跨模块依赖，存在时在胶囊中插入确认步骤
- 确认步骤的查询命令必须具体到文件路径和关键字，不能留给 Code 自行判断
- Code 必须粘贴查询结果后才能继续，不得跳过

### Code 责任

- 发现接口不一致时，必须停下来申报，不得自行猜测兼容方案
