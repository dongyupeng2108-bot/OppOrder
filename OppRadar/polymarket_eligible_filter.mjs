 ...
 * Applies four hard eligibility filters to a list of normalized Polymarket markets.
 *
 * Four hard filters (ALL must pass to be a candidate):
 *  0. Settled market exclusion — yes_price === 0 or yes_price === 1 (market already resolved)
 *  1. Has explicit settlement time — event_time exists and is valid ISO8601
 *  2. TTL30d window — event_time - now <= 30 days
 *  3. Valid price — yes_price ∈ (0.05, 0.95)
 *  4. Candidate cap — output ≤ 50 (sorted by |yes_price - 0.5| ascending, take top 50)
 *
 * Exports:
...
/**
 * Apply all four eligibility filters.
 *
...
export function applyEligibleFilter(markets) {
  ...
  for (const market of markets) {
    ...
    // Filter 1: Has valid ISO8601 settlement time
    if (!isValidIso8601(market.event_time)) {
      breakdown.no_event_time++;
      continue;
    }

    // Filter 2: TTL30d — event_time must be within 30 days from now
  ...
  // Filter 4: Cap at 50 — sort by proximity to 0.5 (most uncertain = most interesting)
