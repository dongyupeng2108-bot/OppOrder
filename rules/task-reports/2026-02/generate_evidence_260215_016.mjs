...
async function main() {
    ...
    try {
        ...
        // 3. Smoke Test Logic
        const runId = `smoke_${TASK_ID}_${Date.now()}`;
        console.log(`Smoke Run ID: ${runId}`);

        // A. Generate Run
        const genUrl = `http://localhost:${PORT}/opportunities/rank_v2?provider=mock&limit=5&run_id=${runId}`;
        const genRes = await fetchUrl(genUrl);
        if (genRes.statusCode !== 200) throw new Error(`Generation failed: ${genRes.statusCode}`);
