/**
 * generate_evidence_260307_011.mjs
 * Evidence generator for Task 260307_011:
 *   B1-a volatility_engine + bs_pricer implementation
 */
...
const resultData = {
  task_id: TASK_ID,
  status: 'DONE',
  summary: 'B1-a: volatility_engine calcVolatility (rolling window annualized HV) + bs_pricer calcBSPrices (Black-Scholes binary option pricing with normCDF A&S 26.2.17). 16/16 unit tests passing.',
  gate_light_exit: 0,
...
const notifyLines = [
  ...
  '',
  'LOG_HEAD',
  `[${TASK_ID}] B1-a volatility_engine + bs_pricer`,
  `Branch: ${branch}`,
  `Commit: ${shortSha}`,
  `Generated: ${now}`,
  '',
  'LOG_TAIL',
  `Task ${TASK_ID} completed successfully. 16/16 tests passed.`,
  '',
  '=== DOD_EVIDENCE_STDOUT ===',
...
writeEvidence(`dod_evidence_${TASK_ID}.txt`, [
  ...
  '',
  '# DoD Markers',
  'DOD_VOLATILITY_ENGINE=calcVolatility implemented (rolling window, log returns, sample variance, annualized)',
  'DOD_BS_PRICER=calcBSPrices implemented (normCDF A&S 26.2.17, d2 formula, pUp/pDown)',
  'DOD_TESTS=16/16 unit tests passing (volatility 5 + bs_pricer 11)',
  '',
  `DOD_EVIDENCE_HEALTHCHECK_ROOT: ${TASK_ID}_healthcheck_53122_root.txt`,
  `DOD_EVIDENCE_HEALTHCHECK_PAIRS: ${TASK_ID}_healthcheck_53122_pairs.txt`,
  '',
...
writeEvidence(`trae_report_snippet_${TASK_ID}.txt`, [
  ...
  `COMMIT: ${shortSha}`,
  `STATUS: DONE`,
  `SUMMARY: B1-a volatility_engine + bs_pricer (16/16 tests)`,
  '',
  '=== DOD_EVIDENCE_STDOUT ===',
  `DOD_EVIDENCE_HEALTHCHECK_ROOT: ${TASK_ID}_healthcheck_53122_root.txt`,
  `DOD_EVIDENCE_HEALTHCHECK_PAIRS: ${TASK_ID}_healthcheck_53122_pairs.txt`,
