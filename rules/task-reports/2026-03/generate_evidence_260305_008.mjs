/**
 * generate_evidence_260305_008.mjs
 * Evidence generator for Task 260305_008:
 *   Fix /scans/run always using mock — provider param + getProvider('live')
 */
...
const reportContent = JSON.stringify({
  task_id:     TASK_ID,
  title:       'Fix /scans/run live provider routing',
  status:      'PASS',
  deliverables: [
    'OppRadar/mock_server_53122.mjs — provider param added to destructure + fallback chain',
    'OppRadar/llm_provider.mjs — case live added to getProvider switch',
  ],
  verification:        'POST /scans/run {provider:live} → deepseek-chat, 11.7s latency, fallback_count=0',
  gate_light_exit:     0,
...
const dodLines = [
  ...
  '',
  '# DoD Markers',
  'DOD_PROVIDER_PARAM=provider added to runScanCore destructure + fallback chain',
  'DOD_LIVE_CASE=getProvider switch includes case live → DeepSeekProvider',
  'DOD_VERIFIED=POST /scans/run {provider:live} returns provider=live model=deepseek-chat',
  'DOD_NO_FALLBACK=fallback_count=0 (no mock fallback)',
  '',
  'GATE_LIGHT_EXIT=0',
].join('\n');
