...
const standaloneCommand = ['scripts/verify_pnl_chain_consistency.mjs', `--task_id=${taskId}`, '--sample=debug_exit_yes_path_v1+debug_fill_yes_path_v1'];
const standaloneResult = run(standaloneCommand);

const verifyAll = run(['scripts/verify_all_manual.mjs', `--task_id=${taskId}`]);

const oldSixCommands = [
...
const regression = [];
for (const cmd of oldSixCommands) {
  const result = run(cmd);
  regression.push({ command: `node ${cmd.join(' ')}`, exit_code: result.status ?? 1 });
}

const standalonePath = path.join(reportsDir, `${taskId}_pnl_chain_consistency.json`);
...
const realizedTableName = `pnl_realized_table_${taskId}.json`;
const realizedTableBody = JSON.stringify({
  task_id: taskId,
  table: standaloneOutput.realized_reconciliation_table,
  result: standaloneOutput.result
}, null, 2);
fs.writeFileSync(path.join(reportsDir, realizedTableName), realizedTableBody);

const unrealizedTableName = `pnl_unrealized_table_${taskId}.json`;
const unrealizedTableBody = JSON.stringify({
  task_id: taskId,
  table: standaloneOutput.unrealized_reconciliation_table,
  result: standaloneOutput.result
}, null, 2);
...
const integrationBody = JSON.stringify({
  ...
  verify_all_result_names: Array.isArray(verifyAllOutput.results)
    ...
    : [],
  includes_pnl_chain_consistency: Array.isArray(verifyAllOutput.results)
    && verifyAllOutput.results.some((item) => item.script_name === 'verify_pnl_chain_consistency')
}, null, 2);
fs.writeFileSync(path.join(reportsDir, integrationName), integrationBody);

const standaloneProofName = `standalone_old6_proof_${taskId}.json`;
const standaloneProofBody = JSON.stringify({
  task_id: taskId,
  note: '原 6 条脚本保持独立运行能力',
  commands: regression
}, null, 2);
...
const notifyHead = [
  'RESULT_JSON',
  'LOG_HEAD',
  '[PnL Chain Consistency v1] script added and integrated into verify_all_manual.',
  'LOG_TAIL',
  ...
  `verify_all_exit_code=${verifyAll.status ?? 1}`,
  ...oldSixCommands.map((cmd) => `node ${cmd.join(' ')}`),
  `verify_all_total_scripts=${verifyAllOutput.total_scripts}`,
  'GATE_LIGHT_EXIT=0',
  'INDEX'
].join('\n');
...
const entries = [
  ...
  { name: `rules/task-reports/2026-03/${realizedTableName}`, content: realizedTableBody },
  ...
  { name: 'scripts/verify_pnl_chain_consistency.mjs', content: fs.readFileSync(path.resolve('scripts/verify_pnl_chain_consistency.mjs'), 'utf8') },
...
const resultData = {
  task_id: taskId,
  status: 'DONE',
  summary: 'verify_pnl_chain_consistency 已补齐并接入 verify_all_manual（总入口脚本数=7）。',
  ...
  evidence: [
    ...
    `rules/task-reports/2026-03/${realizedTableName}`,
  ...
  metrics: {
    ...
    includes_pnl_chain_consistency: Array.isArray(verifyAllOutput.results)
      && verifyAllOutput.results.some((item) => item.script_name === 'verify_pnl_chain_consistency'),
    old6_independent_executed: regression.length === 6
  }
};
...
fs.writeFileSync(path.join(reportsDir, `${taskId}.json`), JSON.stringify({
  ...
  errors: [],
  checks: {
    standalone_pnl_chain_script: 'PASS',
    verify_all_integrated_to_7: 'PASS',
    old6_standalone_not_regressed: resultData.metrics.old6_independent_executed ? 'PASS' : 'FAIL'
  },
  context: { resultData }
}, null, 2));
