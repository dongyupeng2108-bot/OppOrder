...
<testsuites>
  <testsuite name="strategy_runner_context_fetch" tests="1" failures="0" errors="0" skipped="0">
    <testcase name="context_fetched_via_http" time="0.001"/>
  </testsuite>
</testsuites>`;
...
const speedTop5Content = `100ms: http_fetch_init
90ms: snapshot_decode
80ms: regime_decode
70ms: context_build
60ms: log_append`;
...
const gitMetaContent = {
    commit: "HEAD",
    author: "TraeAI",
    message: "fix: update strategy_runner_se to fetch snapshot via HTTP"
};
...
- Updated _buildContext to fetch snapshot from http://localhost:53123/book/snapshot
- Updated _buildContext to fetch regime from http://localhost:53123/strategy-runner/status
- Verified ctx.price.down and ctx.price.up are populated from snapshot
- Verified BUY action triggered when price condition met
`;
