/**
 * generate_evidence_260305_010.mjs
 * Evidence generator for Task 260305_010:
 *   Fix report prompt PR number detection — number + pr_number compat
 */
...
const reportContent = JSON.stringify({
  task_id:     TASK_ID,
  title:       'Fix report prompt PR number detection',
  status:      'PASS',
  deliverables: [
    'scripts/run_task.ps1 — report prompt reads both number and pr_number fields',
    'scripts/run_task.ps1 — uses Get-Content | ConvertFrom-Json instead of Read-JsonSafe',
  ],
  verification:        'PowerShell syntax PASS; PR# correctly resolved from auto_pr JSON',
  gate_light_exit:     0,
...
const dodLines = [
  ...
  '',
  '# DoD Markers',
  'DOD_DUAL_FIELD=checks both number and pr_number in auto_pr JSON',
  'DOD_GET_CONTENT=uses Get-Content | ConvertFrom-Json (not Read-JsonSafe)',
  'DOD_SYNTAX_OK=PowerShell PSParser returns 0 errors',
  'DOD_NO_BREAK=existing pipeline unaffected',
  '',
  'GATE_LIGHT_EXIT=0',
].join('\n');
