...
const resultData = {
  task_id: TASK_ID,
  status: 'DONE',
  summary: 'B1-b: price_feed (Binance REST getCurrentPrice/getKlines/polling) + market_scanner (Polymarket Gamma API findCurrentWindow/findNextWindow). Integration tests created.',
  gate_light_exit: 0,
...
const notifyLines = [
  ...
  '',
  'LOG_HEAD',
  `[${TASK_ID}] B1-b price_feed + market_scanner`,
  `Branch: ${branch}`,
  `Commit: ${shortSha}`,
  `Generated: ${now}`,
  '',
...
writeEvidence(`dod_evidence_${TASK_ID}.txt`, [
  ...
  '',
  '# DoD Markers',
  'DOD_PRICE_FEED=createPriceFeed factory with getCurrentPrice/getKlines/startPolling/stopPolling',
  'DOD_MARKET_SCANNER=createScanner factory with findCurrentWindow/findNextWindow',
...
writeEvidence(`trae_report_snippet_${TASK_ID}.txt`, [
  ...
  `COMMIT: ${shortSha}`,
  `STATUS: DONE`,
  `SUMMARY: B1-b price_feed + market_scanner`,
  '',
  '=== DOD_EVIDENCE_STDOUT ===',
  `DOD_EVIDENCE_HEALTHCHECK_ROOT: ${TASK_ID}_healthcheck_53122_root.txt`,
  `DOD_EVIDENCE_HEALTHCHECK_PAIRS: ${TASK_ID}_healthcheck_53122_pairs.txt`,
