...
const standaloneCommand = ['scripts/verify_result_chain_consistency.mjs', `--task_id=${taskId}`, '--sample=debug_fill_yes_path_v1'];
...
const regression = [];
for (const cmd of oldFourCommands) {
  const result = run(cmd);
  regression.push({ command: `node ${cmd.join(' ')}`, exit_code: result.status ?? 1 });
}

const standaloneOutputPath = path.join(reportsDir, `${taskId}_result_chain_consistency.json`);
const standaloneOutput = JSON.parse(fs.readFileSync(standaloneOutputPath, 'utf8'));
...
const reconciliationName = `result_chain_table_${taskId}.json`;
const reconciliationBody = JSON.stringify({
  task_id: taskId,
  table: standaloneOutput.result_chain_table,
  checks: standaloneOutput.checks
}, null, 2);
fs.writeFileSync(path.join(reportsDir, reconciliationName), reconciliationBody);
...
const integrationBody = JSON.stringify({
  ...
  verify_all_result_names: Array.isArray(verifyAllOutput.results)
    ...
    : [],
  includes_result_chain_consistency: Array.isArray(verifyAllOutput.results)
    && verifyAllOutput.results.some((item) => item.script_name === 'verify_result_chain_consistency')
}, null, 2);
fs.writeFileSync(path.join(reportsDir, integrationName), integrationBody);

const standaloneProofName = `standalone_old4_proof_${taskId}.json`;
const standaloneProofBody = JSON.stringify({
  task_id: taskId,
  note: '原 4 条脚本保持独立运行能力',
  commands: regression
}, null, 2);
...
const notifyHead = [
  'RESULT_JSON',
  'LOG_HEAD',
  '[Result Chain Consistency v1] script added and integrated into verify_all_manual.',
  'LOG_TAIL',
  `node ${standaloneCommand.join(' ')}`,
  `node scripts/verify_all_manual.mjs --task_id=${taskId}`,
  ...oldFourCommands.map((cmd) => `node ${cmd.join(' ')}`),
  `verify_all_total_scripts=${verifyAllOutput.total_scripts}`,
  'GATE_LIGHT_EXIT=0',
  'INDEX'
].join('\n');
...
const entries = [
  { name: `rules/task-reports/2026-03/${taskId}_result_chain_consistency.json`, content: fs.readFileSync(path.join(reportsDir, `${taskId}_result_chain_consistency.json`), 'utf8') },
  { name: `rules/task-reports/2026-03/${taskId}_result_chain_consistency.log`, content: fs.readFileSync(path.join(reportsDir, `${taskId}_result_chain_consistency.log`), 'utf8') },
  ...
  { name: `rules/task-reports/2026-03/${reconciliationName}`, content: reconciliationBody },
  ...
  { name: 'scripts/verify_result_chain_consistency.mjs', content: fs.readFileSync(path.resolve('scripts/verify_result_chain_consistency.mjs'), 'utf8') },
...
const resultData = {
  task_id: taskId,
  status: 'DONE',
  summary: 'verify_result_chain_consistency 已补齐并接入 verify_all_manual（总入口脚本数=5）。',
  report_file: notifyName,
  report_sha256_short: hash8(notifyBody),
  evidence: [
    `rules/task-reports/2026-03/${taskId}_result_chain_consistency.json`,
    `rules/task-reports/2026-03/${taskId}_result_chain_consistency.log`,
    `rules/task-reports/2026-03/${reconciliationName}`,
  ...
  metrics: {
    ...
    includes_result_chain_consistency: Array.isArray(verifyAllOutput.results)
      && verifyAllOutput.results.some((item) => item.script_name === 'verify_result_chain_consistency'),
    old4_independent_executed: regression.length === 4
  }
};
...
fs.writeFileSync(path.join(reportsDir, `${taskId}.json`), JSON.stringify({
  ...
  errors: [],
  checks: {
    standalone_result_chain_script: 'PASS',
    verify_all_integrated_to_5: 'PASS',
    old4_standalone_not_regressed: resultData.metrics.old4_independent_executed ? 'PASS' : 'FAIL'
  },
  context: { resultData }
}, null, 2));
