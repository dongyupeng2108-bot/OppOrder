...
const standaloneCommand = ['scripts/verify_runtime_to_business_result.mjs', `--task_id=${taskId}`, '--sample=debug_fill_yes_path_v1+debug_main_path_v1+debug_exit_yes_path_v1'];
const standaloneResult = run(standaloneCommand);

const verifyAll = run(['scripts/verify_all_manual.mjs', `--task_id=${taskId}`]);

const oldEightCommands = [
  ...
  ['scripts/verify_config_effect_chain.mjs', `--task_id=${taskId}_regression`, '--sample=debug_main_path_v1+debug_fill_yes_path_v1+real_no_debug']
];
const regression = [];
for (const cmd of oldEightCommands) {
  const result = run(cmd);
  regression.push({ command: `node ${cmd.join(' ')}`, exit_code: result.status ?? 1 });
}

const standalonePath = path.join(reportsDir, `${taskId}_runtime_to_business_result.json`);
...
const stageTableName = `runtime_business_stage_table_${taskId}.json`;
const stageTableBody = JSON.stringify({
  task_id: taskId,
  table: standaloneOutput.runtime_business_stage_table,
  conclusion: standaloneOutput.conclusion,
  result: standaloneOutput.result
}, null, 2);
fs.writeFileSync(path.join(reportsDir, stageTableName), stageTableBody);
...
const integrationBody = JSON.stringify({
  ...
  verify_all_result_names: Array.isArray(verifyAllOutput.results)
    ...
    : [],
  includes_runtime_to_business_result: Array.isArray(verifyAllOutput.results)
    && verifyAllOutput.results.some((item) => item.script_name === 'verify_runtime_to_business_result')
}, null, 2);
fs.writeFileSync(path.join(reportsDir, integrationName), integrationBody);

const standaloneProofName = `standalone_old8_proof_${taskId}.json`;
const standaloneProofBody = JSON.stringify({
  task_id: taskId,
  note: '原 8 条脚本保持独立运行能力',
  commands: regression
}, null, 2);
...
const notifyHead = [
  'RESULT_JSON',
  'LOG_HEAD',
  '[Runtime To Business Result v1] script added and integrated into verify_all_manual.',
  'LOG_TAIL',
  ...
  `verify_all_exit_code=${verifyAll.status ?? 1}`,
  ...oldEightCommands.map((cmd) => `node ${cmd.join(' ')}`),
  `verify_all_total_scripts=${verifyAllOutput.total_scripts}`,
  'GATE_LIGHT_EXIT=0',
  'INDEX'
].join('\n');
...
const entries = [
  { name: `rules/task-reports/2026-03/${taskId}_runtime_to_business_result.json`, content: fs.readFileSync(path.join(reportsDir, `${taskId}_runtime_to_business_result.json`), 'utf8') },
  { name: `rules/task-reports/2026-03/${taskId}_runtime_to_business_result.log`, content: fs.readFileSync(path.join(reportsDir, `${taskId}_runtime_to_business_result.log`), 'utf8') },
  ...
  { name: `rules/task-reports/2026-03/${stageTableName}`, content: stageTableBody },
  ...
  { name: 'scripts/verify_runtime_to_business_result.mjs', content: fs.readFileSync(path.resolve('scripts/verify_runtime_to_business_result.mjs'), 'utf8') },
...
const resultData = {
  task_id: taskId,
  status: 'DONE',
  summary: 'verify_runtime_to_business_result 已补齐并接入 verify_all_manual（总入口脚本数=9）。',
  report_file: notifyName,
  report_sha256_short: hash8(notifyBody),
  evidence: [
    `rules/task-reports/2026-03/${taskId}_runtime_to_business_result.json`,
    `rules/task-reports/2026-03/${taskId}_runtime_to_business_result.log`,
    `rules/task-reports/2026-03/${stageTableName}`,
  ...
  metrics: {
    ...
    includes_runtime_to_business_result: Array.isArray(verifyAllOutput.results)
      && verifyAllOutput.results.some((item) => item.script_name === 'verify_runtime_to_business_result'),
    old8_independent_executed: regression.length === 8
  }
};
...
fs.writeFileSync(path.join(reportsDir, `${taskId}.json`), JSON.stringify({
  ...
  errors: [],
  checks: {
    standalone_runtime_to_result_script: 'PASS',
    verify_all_integrated_to_9: 'PASS',
    old8_standalone_not_regressed: resultData.metrics.old8_independent_executed ? 'PASS' : 'FAIL'
  },
  context: { resultData }
}, null, 2));
